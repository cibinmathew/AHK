...
SYS_TrayTipShow:
	...
	{
		If ( !SYS_TrayTipTitle )
			SYS_TrayTipTitle = nifty ; %SYS_ScriptInfo%
...
SUS_SuspendToggle:
	...
	If ( !A_IsSuspended )
	{
		...
		Progress, b fs18 zh0, suspended, , , Calibri
        Sleep, 750
        Progress, off
	...
	Else
	{
	    Progress, b fs18 zh0, activated, , , Calibri
        Sleep, 750
        Progress, off
		Suspend, Off
		...
		If ( (SUS_WinMinMax = 0) and (SUS_WinX = 0) and (SUS_WinY = 0) and (SUS_WinW = A_ScreenWidth) and (SUS_WinH = A_ScreenHeight) )
		{
			...
			If ( (SUS_WinClass != "Progman") and (SUS_ProcessExt != "scr") and !SUS_FullScreenSuspend )
			{
				...
				If ( !A_IsSuspended )
				{
					...
					SYS_TrayTipText = full screen suspension
					;SYS_TrayTipOptions = 2
					Gosub, SYS_TrayTipShow
					Gosub, TRY_TrayUpdate
				}
...
;$#RButton::	; na
	; NWD_ResizeGrids = 7
	NWD_ResizeGrids = 4	; increase to trigger only on the ends
	CoordMode, Mouse, Screen
			...
			If ( NWD_Dragging )
			{
				...
				WinMove, ahk_id %NWD_WinID%, , %NWD_WinNewX%, %NWD_WinNewY%, %NWD_WinNewW%, %NWD_WinNewH%
					...
					If ( SYS_ToolTipFeedback )
					{
						...
						;SYS_ToolTipText = (X:%NWD_ToolTipWinX%, Y:%NWD_ToolTipWinY%) 
						;SYS_ToolTipText = (X:%NWD_ToolTipWinX%, Y:%NWD_ToolTipWinY%) 
						if (NWD_WinNewW != NWD_WinW) or (NWD_WinNewH != NWD_WinH)
						   {						   
						   SYS_ToolTipText = (X:%NWD_ToolTipWinX%, Y:%NWD_ToolTipWinY%) 
							Gosub, SYS_ToolTipFeedbackShow
							}
...
AOT_SetToggle:
	...
	IfWinActive, A
	{
		...
		{
			SYS_ToolTipText = AOT: OFF
			Gosub, AOT_SetOff
		}
		Else
		{
			SYS_ToolTipText = AOT: ON
			Gosub, AOT_SetOn
		}
		Gosub, SYS_ToolTipFeedbackShow
	}
...
AOT_SetOn:
	...
	; msgbox,%a_thishotkey%
	IfInString, A_ThisHotkey, capslock	;	title not changed for this hotkey
		return
	WinGetTitle, TempText, A
	If (SubStr(TempText, 1,3) != "†† ")
...
Return
 ...
 #+MButton::	; na
	...
	IfInString, A_ThisHotkey, MButton
	{
		AOT_WinID = %TRA_WinID%
		Gosub, AOT_SetOn
		TRA_WinAlpha%TRA_WinID% := 35 * 255 / 100
	}
...
TRY_TrayEvent:
	...
	If ( TRY_TrayEvent = "About" )
	{
	 ...
	 SplashImage, %A_Script_Drive%\cbn\ahk\old\Untitled.png, b fs18
     Sleep, 300
     SplashImage, Off
